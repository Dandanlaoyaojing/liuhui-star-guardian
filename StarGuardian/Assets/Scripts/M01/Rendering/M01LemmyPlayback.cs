...
namespace StarGuardian.M01.Rendering
{
    public static class M01LemmyPlayback
    {
        ...
        public static int[] FrameRangeIndices(
            string actionId,
            ...
            bool reverse)
        {
            ValidateFrameRange(loadedFrameCount, sourceStartFrame, sourceFrameCount);
            var playable = PlayableFrameIndices(actionId, loadedFrameCount);
            var sourceEndFrame = sourceStartFrame + sourceFrameCount;
            var selectedCount = 0;
            foreach (var sourceFrame in playable)
            {
            ...
            foreach (var sourceFrame in playable)
            {
                if (sourceFrame >= sourceStartFrame && sourceFrame < sourceEndFrame)
                {
                    result[resultIndex++] = sourceFrame;
                }
            }
        ...
        public static double[] FrameRangeDurationsMs(
            string actionId,
            int loadedFrameCount,
            int sourceStartFrame,
            int sourceFrameCount,
            bool reverse)
        {
            ValidateFrameRange(loadedFrameCount, sourceStartFrame, sourceFrameCount);
            var playable = PlayableFrameIndices(actionId, loadedFrameCount);
            var durations = FrameDurationsMs(actionId, loadedFrameCount);
            var sourceEndFrame = sourceStartFrame + sourceFrameCount;
            var selectedCount = 0;
            foreach (var sourceFrame in playable)
            {
            ...
            var result = new double[selectedCount];
            var resultIndex = 0;
            for (var index = 0; index < playable.Length; index += 1)
            {
                var sourceFrame = playable[index];
                if (sourceFrame >= sourceStartFrame && sourceFrame < sourceEndFrame)
                {
                    result[resultIndex++] = durations[index];
                }
            }
            if (reverse) Array.Reverse(result);
            return result;
            ...
            int sourceFrameCount)
        {
            ...
            {
                throw new ArgumentOutOfRangeException(nameof(sourceStartFrame));
            }
            if (sourceFrameCount <= 0)
            {
                throw new ArgumentOutOfRangeException(nameof(sourceFrameCount));
            }
            if (sourceStartFrame > loadedFrameCount - sourceFrameCount)
            {
                throw new ArgumentOutOfRangeException(nameof(sourceFrameCount));
            }
        }
