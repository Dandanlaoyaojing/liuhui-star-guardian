...
- **验证**: `dotnet test` **361/361**；Unity 生成的 `Assembly-CSharp.csproj` **0 error**（完整构建仍有仓库既有 nullable/重复 using 警告）；`npm audit --omit=dev` **0 vulnerability**。Cocos 全套当前为 **479/480**：唯一失败是既有 `LemmyActor.test.ts` 仍断言 17 动作，而当前表已含第 18 个 `nodside`，与本轮 Unity 改动无关。Unity Editor Play 已实走真实篮内堆、三次各 3 片、曲线前沿、两股 12px 麻绳，以及首批落地拼片的轻点旋转/拖起自由落下；“不在篮下直接点篮子只能走近、伸手、转脸摇头”由 `M01IntroFlowTests` 双阶段回归桩锁定。2026-07-14 另在“兔子到位但 roam 仍处收尾帧”的最窄竞态窗口点击篮子，现场确认 `headbutt` 不再被覆盖且第一批只掉 3 片；修正 PTM32 换算后现场捕获首批拼片上扬约 170px 的中途帧并确认继续回落碰撞，理论峰值约 320px。手电对象激活、坐标、SpriteRenderer 启用状态已在 Unity Hierarchy/Inspector 实查。最新 alpha-trim 补偿已由尺寸契约回归锁定；自然落地回归同时锁定砸头后不再改写手电的位置、旋转、速度、旋转约束或休眠状态；拾取分支回归锁定地面蹲拾与拼片上站立拿取；左右来向回归锁定不跨过手电且到位面向道具。圆片 `60×60` 美术显示、其余形状 `56×56` 以及标准拼接几何不变由形状尺寸与 Unity 胶水契约回归锁定。纸底、Unlit 材质、三角光锥、六种精确显色值和齿轮/篮子 Linear 局部补偿由视觉等价测试及 Unity Play 运行态读值双重验证。
