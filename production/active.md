...
## 十星航线收束 + 光与雾世界观(2026-07-18, 用户拍板, spec v2.0)
**全游戏 = 正典十关五幕(3+2+2+2+1), 33 关索引降级为题材库。**权威=spec §2.3(十星表)+§1.4(世界观与价值观——用户点名必须写入的核心)。要点: ①反派"迷雾"吃关系不吃实体=智力定义(找最底层关系)的精确反面; 五条可玩性法则+五幕雾相升级史(拆解/缠结/湍流/镜中/本源); 雾永不说话永无面孔(姿态铁律防动画片化); 每关立项须答"雾的招式落在哪个可交互行为上"+立项三问。②价值观四支柱: 智慧是关系的看见/成长可解决一切困难/成长的终极是帮助别人成长/守护是持续实践——与 bunnies-dreamworld 同宇宙(迷雾=遗忘)。③莱米弧: 被点亮(M01 被给予→M02 被温暖)→传灯关第一次给予("分光不减光")→终章成为点亮者(教星星自己点灯)。④M01/M02 零机制改动只补叙事追认(M01 溢出星光化流星=M02 序章三余烬来历)。
**席 3 =《休眠的传灯道》(M06 全节重设, spec §5.7)**: 因果排序; 有向灯驿图+魔杖电量紧配额; 顿悟点"空间上近≠因果上先"; 三板(一线灯/回折的驿道/双源汇灯+诱饵支路); 序章七拍含"分光不减光"; config 命名 m06-lantern-relay.json。**下一开发目标=席3 greybox**。
**成长表达(用户两轮校准后定稿, §1.4.6)**: 只留"可见账本"——星图领土永亮(雾对锁形免疫)+流辉美慧号以结晶为燃料逐关变亮; **安静原则**(无文字无提示无奖励音, 玩家自己发现)。用户否掉了我提的"每关成长七拍"文法(嫌刻意/检查表化)——好关卡自然长出无力拍与失败即教学, 作惯例参照不作立法; 同机位回望镜头=摄影式说教, 已弃。
**道层拍板(2026-07-18 第二轮, §1.4.7+§2.3 道列)**: 用户点破"认知工具是术, 我要的是人生智慧(道)"——每关双层: 术=机制载体, 道=成长历程正身。十道=明辨/团结/给予/逆袭/坚韧/顺势/机缘/独立思考/兼容并包/舍得与传道(弧线 3+3+3+1: 看清与相连→逆境三课→处世三课→终课)。**换席两处**: 席7 概率云室(M22)→时序档案馆(M21,承"机缘"), 席9 光子星(M24)→悖论果园(M20,承"兼容并包")——原席8/9待复议就此关闭。**倾光结局入典(§1.4.5)**: 终章莱米把整船流辉倾倒进群星、船暗星亮=勇于牺牲+传道+范式转换三合一。结晶写作准则追加"两层都读得出"(§2.4)。
**codex 零框架盲审已消费(2026-07-18, 10 findings)**: 采纳硬伤 2(CLAUDE.md 席7/9 素材源仍写换席前 M22/M24→已改 M21/M20; spec §1.3 引擎口径仍 Cocos/TS + M01 过场仍 Android/Web→已改 Unity/C#/iOS+Steam)。小修 5(传灯"正解只有一条"→"一类,合法拓扑序不止一条"; §1.4.2 补雾法则生效边界=席3起、席1-2匿名期为有意设计; §1.4.5 结局标注机制以终章危险原型为准; §1.4.6 补账本存档派生口径; M03 席位注标记"情绪气体中和"无定义、立项补全或裁掉)。驳回 2(M01/M02 雾不可玩=匿名期叙事设计; M01 术名"分类归纳"不匹配=归纳在证据反推主链上成立, ToolCard 文案注意)。已被规格覆盖不动 2(M08/M05 雾待细化=席位注明示; 终章无机制=危险原型门槛在)。
**M06 电量语义已拍板翻转(2026-07-18, 用户选定)**: 正确分光零成本("分光不减光"成为机制事实), 星尘只在错序被雾吞时扣(容错预算约 2/板, 求解器口径改为"0 错可通+诱饵确为死路", 胜利条件 charges→mistakeBudget)。与 M02 语义差为有意对照(唤醒要花力气, 给予不用)。spec §5.7 已全面改写电量相关段落。
**改动文件**: game-design-spec.md(v1.9→2.0, §1.4 新章/§2.3 正典表/§5 分流/§5.7 重写/§7 路线修订), vision.md, CLAUDE.md(Game Scope)。未提交。
...
**标注**: `StarGlowLineWidthPx` 标"Unity 不消费"(Cocos 环形 stroke 三轴 → Unity 径向填充只剩两轴, 技法升级的有意舍弃)。
...
**Play 全流程实测通过(2026-07-16)**: 序章(拔棒→拖余烬聚簇→邻接点亮→达冻结阈值→棒点冻结余烬→done)→ 交接主盘(序章退场, 星层 18 子节点/电量计 3 格)→ 点星修复流(点 A: A frozen life3 + 四邻 B/E/F/I 扩散 decaying, 电量 3→2; 点 F 同理)→ 渲染同步正确(金色暖光星/灰墨暗星/电量计递减/辉光照亮整片星图)。截图存 `docs/design/render-parity/2026-07-16-unity-m02-{prologue,starweb-lit}.png`。**光效达成迁移动机**: Light2D+加法辉光+Bloom 三层, Cocos 平面 Graphics 圆环做不出的光晕扩散。
**测法教训**: 直接调 `Session.TapNode()` 绕过探针 `OnRelease` → 逻辑变了但不重绘, 别误判成渲染 bug; 必须走探针输入路径(渲染同步与 TS `tapNode→renderStars` 逐字一致)。
...
- **篮内物理与颜色管线继续收口**: Cocos 在 `Settling/Headbutting` 阶段始终让九片留在篮子局部坐标系、真正释放时才改挂场景根；Unity 已删除“刚启用 Dynamic 就提前改挂根节点”的偏差。平台、篮子、拼片、绳钉、手电和运行时光效全部显式使用 `Sprite-Unlit-Default`，不再受 URP 2D 默认 Lit 材质/全局灯改变颜色；纸底恢复 Cocos 精确 `(247,244,235)`。手电移除会把色相冲白的 Point Light2D，改为 Cocos 同构的 128px 运行时渐变三角光锥 + 14px 白热灯芯，大头光晕恢复局部 Y=11px/直径18px。新增引擎无关 `M01VisualParity`，逐字迁移光束 RGB、通道相乘与 1.4 倍饱和算法；Unity Play 实测红灯下红片=`(255,41,22)`、黄片混橙=`(222,150,94)`，黄灯下红/黄/蓝片分别=`(222,150,94)/(255,205,13)/(129,171,132)`，蓝灯下分别=`(173,136,172)/(129,171,132)/(38,94,245)`，与 Cocos 字节真值一致。
- **显色与验证已分流**: 拖起、自由落下、弱吸附、目标槽吸附不再写灰/蓝交互色，也不再覆盖原始水彩底色；手电仍按红→黄→蓝→灭循环，光池内候选片走 Session `revealedColor` 实时显色。六个目标槽全占满时，即使有错误角度片只进入 `stick_fragment_to_slot`，也立即触发结构验证；目标图案的证据对按槽位上的**实际占用拼片**组成，不再反查真解 id，因此错色候选会按玩家刚拼出的颜色进入 `wrong_blend_color`/`wrong_fragment_set`。失败按当前拼片真色及真实几何交叠绘制反应色，保持配置的 3 秒后退片；正确保持底光/交叠混色后进入完成演出。
- **拼片输入/物理胶水补齐**: Unity 不再用旧探针的“按住拖、R 键旋转”。现在按真实 GameObject 世界位置命中；按下即关闭碰撞并接管刚体，保留偏心抓取；移动 `≤6px` 原地顺时针转 90°并 Kinematic 钉住 2s，超时无人再抓才结算；真正拖动按拼片视觉中心吸附/贴槽/自由释放，自由落点恢复 Dynamic 物理。最终落堆位置回写 token，稳定后只让 Dynamic 刚体 `Sleep` 而不退出 Physics2D；拿走底层支撑片会唤醒其余拼片重新坍落垒叠。手电拾取前，已释放且位于篮子当前 AABB 外的拼片即可整理。第一次顶篮即撤内胆，未释放拼片继续作为篮子子节点随软绳冲击整体晃动。篮子点击会先取消仍在最后一帧收尾的 roam，避免旧 `idleback` 覆盖刚启动的 `headbutt` 并把输入永久锁死。鼠标和 iOS 触摸统一走 Input System `Pointer`；拾取手电后严格按 `fragment > held flashlight > ground` 路由，点击手持手电循环灯态。
- **帧资源防串入**: `Resources.LoadAll<Sprite>` 显式排除 `*-preview`，预览 GIF 不再把 crouch 40 帧误读成 41 帧。
- **提交后复审修复**: 725 张 Lemmy PNG 使用 `CompressedHQ`（静态篮子/平台等像素级校色资源继续无压缩），动画播放器最多保留 3 个动作的 LRU 缓存并主动卸载淘汰帧，消除“全部动作永久驻留”的内存风险。拼片占用统一收敛到 `M01PlacementLedger`：自由证据严格保留有序的最近两片并覆盖旧提交；目标槽放入新片时会释放旧的错误姿态占用者，但不会挤掉已经姿态正确的拼片；失败回退前统一取消所有延迟旋转钉，避免 2 秒旧协程在复位后重新冻结拼片。
- **开场构图与水彩颜色对齐**: 运行态逐项测量确认篮子源图、`433.44×271.04` displaySize、锚点和 `(300,-32.223)` 位置本来就与 Cocos 相同；上一版并排图因 Unity 仅 `516×344` 内容被放大而产生尺寸错觉，因此未反向篡改正确缩放。删除 Unity 独自手绘的左上完整参考图（Cocos live 的同名根节点无子节点且 fill/stroke alpha 均为 0）。Cocos 齿轮源真值仍保留白色 alpha `210`，但 Unity 项目必须为 URP 2D/Bloom 保持 Linear；其半透明混合和大图缩采样与 Cocos 近 Gamma 的输出不同，直接照搬会让齿轮平均亮度偏亮 `11.4`、篮身偏亮 `1.8`。现仅对齿轮使用 `(255,254,247,232)`、篮身/前沿使用 `(253,251,249,255)` 的局部 Linear 补偿，不影响纸底、拼片、手电显色和全局光效；最终 960×640 同位置测量齿轮平均 RGB 误差约 `(+0.012,-0.081,+0.167)`、亮度 `-0.044`，篮子约 `(+0.275,-0.182,+0.141)`、亮度 `-0.062`。两套 Box2D 求解器从同一掉落种子会收敛到不同静止解，现保留 0.9 秒真实沉降，并只在冻结边界按 fragment id 归一到 Cocos 连续三次预览稳定的九片局部坐标/角度；顶篮时仍恢复 Dynamic Physics2D，释放、上扬、碰撞与落地堆叠不变。Unity Play 运行读值逐片命中，Console 0 error；新对比使用 2×超采样后裁成原生 960×640，篮子、齿轮、兔子、灯泡和地面线几何已对齐。
- **结算已纠偏**: 先 `celebrate` 再播当前 960×640 MP4；当前 Cocos `HIDE_SCREEN_TEXT=true`，因此只保留 ToolCard 逻辑解锁，不额外显示 Unity 旧版大卡 UI。
- **验证(2026-07-14 当时)**: `dotnet test` **373/373**；Unity 生成的 `Assembly-CSharp.csproj` **0 error**（完整构建仍有仓库既有 nullable/重复 using 警告）；`npm audit --omit=dev` **0 vulnerability**。新增回归覆盖最近两片、槽位互斥/替换、失败前取消旋转钉、697 帧压缩元数据和 3 动作 LRU。Cocos 全套当时为 **479/480**：唯一失败是既有 `LemmyActor.test.ts` 仍断言 17 动作，而当前表已含第 18 个 `nodside`，与本轮 Unity 改动无关。Unity Editor Play 已实走真实篮内堆、三次各 3 片、曲线前沿、两股 12px 麻绳，以及首批落地拼片的轻点旋转/拖起自由落下；“不在篮下直接点篮子只能走近、伸手、转脸摇头”由当时的 `M01IntroFlowTests` 回归桩锁定。2026-07-14 另在“兔子到位但 roam 仍处收尾帧”的最窄竞态窗口点击篮子，现场确认 `headbutt` 不再被覆盖且第一批只掉 3 片；修正 PTM32 换算后现场捕获首批拼片上扬约 170px 的中途帧并确认继续回落碰撞，理论峰值约 320px。手电对象激活、坐标、SpriteRenderer 启用状态已在 Unity Hierarchy/Inspector 实查。最新 alpha-trim 补偿已由尺寸契约回归锁定；自然落地回归同时锁定砸头后不再改写手电的位置、旋转、速度、旋转约束或休眠状态；拾取分支回归锁定地面蹲拾与拼片上站立拿取；左右来向回归锁定不跨过手电且到位面向道具。圆片 `60×60` 美术显示、其余形状 `56×56` 以及标准拼接几何不变由形状尺寸与 Unity 胶水契约回归锁定。纸底、Unlit 材质、三角光锥、六种精确显色值和齿轮/篮子 Linear 局部补偿由视觉等价测试及 Unity Play 运行态读值双重验证。复审修复后另做“停止→重新播放”现场复验：域重载和首批帧加载完成后盘面正常出现，Console 无新增脚本异常。
- **复审后的资源清理收口(2026-07-14)**: `M01BoardProbe.OnDisable` 过去先清 `validationOverlaySpriteKeys`，导致本代验证覆盖层运行时生成的 Sprite/Texture 永久失去释放句柄；现拆出 `ReleaseValidationOverlaySprites()`，在 key 仍可达时从 cache 移除并销毁原生资源，同时保留“OnDisable 不强拆场景根节点”的 Unity teardown 约束。已移除 Unity `Resources` 内未被运行时消费的 `startle-preview.gif` / `crouch-preview.gif` 及 meta（约 934KB），并新增回归锁定禁用清理顺序和运行时目录不得再混入预览 GIF。TDD 两条回归均先在旧实现红灯、修后转绿；独立复审发现初版顺序断言不够强，现补成 helper 独占 key 清理且 `cache remove/destroy` 必须早于 `Clear()`，并用故障注入复现“先 clear 再 release”确认测试会红。全套 `dotnet test` **375/375**，Unity `Assembly-CSharp.csproj` **0 error**（54 个既有 warning），`git diff --check` 通过。
- **复审玩法回归收口(2026-07-15)**: ①首次和重复顶篮都要求玩家先点篮下地面让莱米走到位，再点篮触发；走开时点篮只演“够不着”，不得自动回位或推进顶篮。②目标槽已有姿态正确拼片时保留其账本占用，但来片仍停放在槽位，不再被当作否决后交还重力。③拖起与落下统一抬升/恢复拼片本体及全部子渲染器，水彩墨线不会留在下层。④位置耳态在可中断动画前提交，“够不着”收尾及拾灯均按当前位置恢复耳态，拾灯前会终止仍在运行的旧走位。删除为复现临时加入的源码字符串断言；引擎无关测试覆盖相位/槽位账本/渲染排序规则/耳态提交，Unity Editor 验证入口另覆盖真实子 `SpriteRenderer` 排序与来片 `Rigidbody2D` 停放状态。权威 spec 同步明确重复顶篮必须由玩家回位(2026-07-15 用户拍板确认, 与 Cocos 自动回位为有意分歧)与“手电不覆盖位置耳态”。最终验证：xUnit **387/387**、Cocos 对照 **480/480**、Unity Editor 真实组件 **2/2**，TypeScript typecheck、Unity 6.3 batchmode 编译及 `git diff --check` 均通过；依赖未变，沿用本提交前已通过的 npm/NuGet 漏洞检查。
- **齿轮 CONTAIN vs CUSTOM 悬案关闭(2026-07-15, 真机裁决)**: Cocos 预览无头探针实读运行时节点 `M01ArtSprite_gearStar`: contentSize=**581×550.076**(=581×587/620 精确等比 contain)、中心(-120,0)、内容底边设计 Y=**-275.04**(地面-270, 嵌入5px"坐地")。**与 Unity 现状(AddTrimmedArt CONTAIN)逐值一致, 零改动**。审阅 agent 的"CUSTOM 非等比 581×581"推断被真机否——它只看到 Bootstrap 先 setContentSize(581,581), 漏了 334deff 的 aspectContentSize contain 在后覆盖。真机基准截图存 `docs/design/render-parity/2026-07-15-cocos-m01-baseline-960x640.png`(另见: 齿轮中央证据=彩色 overlap 小形状, Unity 现为淡紫圆占位, 视觉差距留正式化)。
- **显色饱和度 1.65 拍板落地(2026-07-15)**: 用户拍板采用 1.65(手电下醒目), 自 2511bac 单独摘出落 main; 有意偏离 Cocos 参照 1.4(不回改)。测试 DisplayName 诚实标注拍板与分歧(不再自称"reproduces Cocos"); evidence 线索色改调 `M01VisualParity.ObservedFragmentTint` 单一真源(保持 Cocos"线索 blend==显色 blend"不变量, 删 BoardProbe 本地 1.4 复算)。`2511bac` 其余内容(光锥偏离/pieceSlotId/mp4/耳态 DoesNotContain)仍不合。
- **正式化生命周期债(集中记档，非当前阻塞)**: ① `M01IntroProbe` 若未来支持“只禁用组件、随后原地恢复”，需成套设计协程停止、阶段快照和重启语义，不能只补 `StopAllCoroutines()`；② `M01LemmyAnimator` 正式化时明确 Sprite 缓存所有权及销毁/场景切换释放策略，避免直接 `UnloadAsset` 误伤共享消费者；③ `M01CompletionProbe` 只有在重新启用屏幕文字 UI（当前 `HideScreenText=true`）前，才需补运行时 Quad Sprite/Texture、动态 Font 与静态 Material 的对称销毁。`Time.fixedDeltaTime=1/60` 已是 `ProjectSettings/TimeManager.asset` 的工程基线，不建立“恢复 0.02”伪债。
- **资源工程决策(正式发布前处理)**: 本波不局部引入 Git LFS，也不把 725 帧盲目打进单张 Atlas。`Resources` 会全部进入 Player 包；合并长期历史/发布前应按全项目资产策略统一决定 LFS 与 Resources→Addressables，并以 Unity Profiler 的包体、峰值纹理内存和批次数据决定是否裁边/拆多张 Atlas。当前播放器按动作延迟加载且 LRU 最多保留 3 个动作，不存在“725 帧同时渲染造成 725 draw call”的路径。
