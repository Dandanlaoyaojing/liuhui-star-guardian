...
- **复审玩法回归收口(2026-07-15)**: ①首次和重复顶篮都要求玩家先点篮下地面让莱米走到位，再点篮触发；走开时点篮只演“够不着”，不得自动回位或推进顶篮。②目标槽已有姿态正确拼片时保留其账本占用，但来片仍停放在槽位，不再被当作否决后交还重力。③拖起与落下统一抬升/恢复拼片本体及全部子渲染器，水彩墨线不会留在下层。④位置耳态在可中断动画前提交，“够不着”收尾及拾灯均按当前位置恢复耳态，拾灯前会终止仍在运行的旧走位。删除为复现临时加入的源码字符串断言；引擎无关测试覆盖相位/槽位账本/渲染排序规则/耳态提交，Unity Editor 验证入口另覆盖真实子 `SpriteRenderer` 排序与来片 `Rigidbody2D` 停放状态。权威 spec 同步明确重复顶篮必须由玩家回位与“手电不覆盖位置耳态”。最终验证：xUnit **387/387**、Cocos 对照 **480/480**、Unity Editor 真实组件 **2/2**，TypeScript typecheck、Unity 6.3 batchmode 编译及 `git diff --check` 均通过；依赖未变，沿用本提交前已通过的 npm/NuGet 漏洞检查。
