...
- **复审四项玩法回归收口(2026-07-15)**: ①重复顶篮阶段莱米若已走开，点篮会先回到顶篮位再继续，不再误走“够不着”分支卡住；首次顶篮仍要求玩家自己走到篮下。②目标槽已有姿态正确拼片时保留其账本占用，但来片仍停放在槽位，不再被当作否决后交还重力。③拖起与落下统一抬升/恢复拼片本体及全部子渲染器，水彩墨线不会留在下层。④位置耳态在可中断动画前提交，拾灯前会终止仍在运行的旧走位，拿到手电不会覆盖篮下收耳。删除为复现临时加入的源码字符串断言；引擎无关测试覆盖相位/槽位账本/渲染排序规则/耳态提交，Unity Editor 验证入口另覆盖真实子 `SpriteRenderer` 排序与来片 `Rigidbody2D` 停放状态。权威 spec 同步明确重复顶篮回位与“手电不覆盖位置耳态”。最终验证：xUnit **387/387**、Cocos 对照 **480/480**、Unity Editor 真实组件 **2/2**，TypeScript typecheck、Unity 6.3 batchmode 编译及 `git diff --check` 均通过；`npm audit` **0 vulnerability**。
