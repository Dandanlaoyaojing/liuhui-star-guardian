...
- **P1-a 安全原型**：M01（分类与归纳）—— 验证基础管线（Cocos Creator + 水彩角色 + 墨线世界 + Klee 配色 + 拖拽交互 + ToolCard 产出）
...
- 搭建 AI 美术 LoRA 训练管线（水彩角色 LoRA + 墨线世界 LoRA）
...
4. **美术分层**：角色水彩（Amanita 血统）+ 世界手绘墨线+饱满色填充（Arrog 只取线条质感）+ 共享 Paul Klee 调色盘。
5. **关键机制**：**认知工具册**（§6.5）—— 每关产出 ToolCard，正面感性（工具名+场景+结晶）/ 反面理性（何时使用+生活例子+常见陷阱）。
6. **技术栈**：Cocos Creator 3.8+ / TypeScript / Dragon Bones 或 Cocos Skeleton2D（Spine Pro 已废弃）。
7. **财务定位**：用户明确"现阶段不考虑成本" —— passion project 优先，商业账延期。
8. **来自 Codex Round 3 的必做验证**：M01 和 Stage 5 原型必须先跑通，否则后续扩写无意义。
