...
- **提交后复审修复**: 697 张 Lemmy PNG 从无压缩改为 `CompressedHQ`（静态篮子/平台等像素级校色资源继续无压缩），动画播放器新增最多 3 个动作的 LRU 缓存并主动卸载淘汰帧，消除“18 动作全部永久驻留”的内存风险。拼片占用统一收敛到 `M01PlacementLedger`：自由证据严格保留有序的最近两片并覆盖旧提交；目标槽放入新片时会释放旧的错误姿态占用者，但不会挤掉已经姿态正确的拼片；失败回退前统一取消所有延迟旋转钉，避免 2 秒旧协程在复位后重新冻结拼片。
...
- **资源工程决策(正式发布前处理)**: 本波不局部引入 Git LFS，也不把 697 帧盲目打进单张 Atlas。帧源约 72.7MiB，且 `Resources` 会全部进入 Player 包；合并长期历史/发布前应按全项目资产策略统一决定 LFS 与 Resources→Addressables，并以 Unity Profiler 的包体、峰值纹理内存和批次数据决定是否裁边/拆多张 Atlas。当前播放器按动作延迟加载且 LRU 最多保留 3 个动作，不存在“697 帧同时渲染造成 697 draw call”的路径。
