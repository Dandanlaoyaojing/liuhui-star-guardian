...
**新版 M01 灰盒已经跑通成功态交互 smoke，失败底光行为的 domain / repo 验证也已补齐，且 repo-local Cocos preview smoke harness 已通过真实预览复验**。执行计划见 `docs/plans/2026-04-29-m01-overlap-evidence-greybox-plan.md`。Task 1-10 已完成：童话颜料混色、真实 overlap evidence JSON、controller/session/domain 校验、layout、drop resolver、session API、Cocos bootstrap 输入接线、legacy sorter art 隔离、ToolCard 与可见文本都已迁到新版“手电 + 证据拼接 + 底光验证 + 关系归纳”路径；Cocos preview 使用 `?scene=a2135734-fc11-4a0e-926d-40bc2301a752` 可稳定进入 `M01Greybox` 场景，并已完成点击拾起 / 点击放置的完整通关 smoke。2026-05-01 已补上“错误整体验证时短暂显露已 staged 碎片本色”的失败底光表现；2026-05-02 已新增并复跑 `npm run smoke:m01-preview`，使用 `playwright-core` + 本机 Chrome/Chromium 检查预览节点是否匹配当前 JSON，并触发错误 complete candidate 验证失败闪光 -> 暂显真色 -> 熄灭回灰；同日已补上首轮 overlap-evidence 运行时贴图：3 张按形状共享的灰白碎片 sprite（circle / triangle / hexagon）和 4 张按 `targetBlendColor + sourceShapes` 映射的 evidence marker sprite，真实 M01 token art 不再只剩 gear token。若 Cocos preview 再次出现旧 bundle，先通过 MCP 刷新 `db://assets/scripts` 与 `db://assets/resources/configs/stage1`，再执行 `sceneAdvanced/soft_reload_scene`，随后复跑 smoke。下一步应对这批首轮 runtime sprite 做视觉精修，统一线条粗细、重叠语义和纸面质感，并在 Cocos 预览里做一次 focused visual QA。
