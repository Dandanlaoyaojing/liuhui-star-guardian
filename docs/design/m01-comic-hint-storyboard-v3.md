# M01 Silent Comic Hint Storyboard v3

Purpose: teach M01's playable actions through Machinarium-style silent comic panels. The final comic must contain no written hints, labels, numbers, speech bubbles, captions, or UI text.
...
| 5 | Lemmy holds a glowing flashlight. A tap/cursor mark lands on empty ground, not on a fragment. A dotted path shows Lemmy moving there while the light cone follows. | Tap empty ground to move Lemmy with the lit flashlight. |
| 6 | The flashlight beam sweeps across grey-white fragments on the ground. Fragments inside the beam become colored; fragments outside remain grey-white. | Sweep fragments with the lit flashlight to reveal their colors. |
| 7 | A cursor taps one fragment. A curved rotation arrow wraps around it; an after-image shows the same fragment rotated to a new angle. | Tap a fragment to rotate it. |
| 8 | Several observed and rotated fragments are placed onto the central platform, overlapping near the target evidence colors. A few unused fragments remain aside. | Arrange only the needed fragments on the platform. |

Hard rule: the final comic is silent. No explanatory text appears inside any panel.
